  ...
  _connect() {
    return new Promise((resolve, reject) => {
      ...
      this.socket = tls.connect(this.opts, () => {
        ...
        const cert = /** @type {crypto.X509Certificate} */ (
          /** @type {any} */ (this.socket.getPeerCertificate(true))
        )
...
Received: "${hash}"`))
          ...
          reject(new Error(this.socket.authorizationError.toString()))
          this.close()
        }
      })
  ...
  async lookup(name, opts = {}) {
    ...
    this.verbose('DNSoverTLS.lookup options:', nopts)
   ...
   */
  _rejectPending(er) {
    for (const pend of Object.values(this.pending)) {
      pend.reject(er)
    }
    this.pending = {}
  }
   ...
   */
  close() {
    const er = new Error('Socket closed')
    return new Promise((resolve, reject) => {
      if (this.socket) {
        this.socket.end(null, () => {
          this._rejectPending(er)
          resolve()
        })
      } else {
        this._rejectPending(er)
